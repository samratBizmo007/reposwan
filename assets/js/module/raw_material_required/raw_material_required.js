...
//                    rawMaterialRequired = JSON.parse(data[i].rm_required);
    ...
    $scope.getPoDataByPo_number = function () {
        $http({
            method: 'get',
            url: BASE_URL + 'required_rawmaterial/Required_rawmaterial/getPoDataByPo_number?po_number=' + $scope.po_number
        }).then(function successCallback(response) {
            // Assign response to skills object
            $scope.po = [];
            var data = response.data;
    ...
    $scope.getPoDataToDropdown = function () {
        ...
        $.ajax({
            type: "GET",
            url: BASE_URL + "required_rawmaterial/Required_rawmaterial/getPoDataByPo_number",
            data: {
                po_number: po_number
            },
            cache: false,
            success: function (data) {
                ...
                var i;
                var data = '<option value="" selected>Select Purchase Orders</option>';
                var emptyOp = $('#po_orders :first-child');
...
function getPoProductDetails() {
    ...
    //
    console.log(po_orders);
    if (po_orders == '? undefined:undefined ?') {
