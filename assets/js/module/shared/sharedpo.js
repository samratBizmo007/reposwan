    ...
    $scope.getSharedPoDetails = function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var i;
                var dat = '<option value="" selected>Select Purchase Orders</option>';
                var emptyOp = $('#sharedpoOrders :first-child');

                emptyOp.nextAll().remove();
                emptyOp.after(dat);
                //$("#sharedpoOrdersSelected").empty();
                for (i = 0; i < podata.length; i++) {
                    $('#sharedpoOrders').append('<option value="' + podata[i].product_code + '/' + podata[i].po_id + '">Order_No- </p>' + podata[i].order_no + ' - Line_No: ' + podata[i].line_no + ' - Part code: ' + podata[i].product_code + ' - Due Date: ' + podata[i].po_duedate + '</option>');
                }
            }
        });
    };
...
//        var sharedpo_orders = $("#sharedpoOrders").val();
//        
//        if (sharedpo_orders === '? undefined:undefined ?') {
//            $("#message").html('<div class="alert alert-warning alert-dismissible fade in alert-fixed w3-round"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Failure!</strong> Please Select The Valid Shared Purchase Order.</div><script>window.setTimeout(function() {	$(".alert").fadeTo(500, 0).slideUp(500, function(){$(this).remove();});}, 2000);</script>');
//            return false;
//        }
...
//            success: function (data) {
//                //$.alert(data);
//                $("#sharedpoOrders").empty();
//                $("#sharedpoOrders").html(data);
//            }
//        });
//    };
